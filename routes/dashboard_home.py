        ...
        try:
            ...
            try:
                ...
                for cli_raw, cod_ent, n_arrivo_val, n_colli, m2_val, peso_val in rows_art:
                    ...
                    rec['righe'] += 1
                    if _is_galvano_cliente(nome_cli):
                        key_pal = _pallet_key_galvano(cod_ent, n_arrivo_val)
                        if key_pal:
                            rec['_galvano_pallet_keys'].add(key_pal)
                        try:
                            rec['_galvano_colli_fallback'] += int(float(n_colli or 0))
                        except Exception:
                            pass
                    else:
                        try:
                ...
                try:
                    ...
                    for cli_b, creati, usciti, aperti in q_b.group_by(BuonoCarico.cliente).all():
                        nome_b = _cliente_dashboard(cli_b, None)
                        buoni_by_cliente[nome_b] = {
                ...
                for nome_cli, rec in sorted(gruppi_clienti.items(), key=lambda x: x[0]):
                    ...
                    if _is_galvano_cliente(nome_cli):
                        keys_pal = rec.get('_galvano_pallet_keys') or set()
                        if keys_pal:
                            rec['colli'] = len(keys_pal)
                        else:
                            rec['colli'] = int(rec.get('_galvano_colli_fallback') or 0)
