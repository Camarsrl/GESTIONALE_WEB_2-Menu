...
def _calcola_logica_giacenza_qty(articoli, data_da, data_a, raggruppamento, qty_field: str = "n_colli"):
    """
    Calcola la GIACENZA nel periodo (giorno o mese) usando una quantità:
    - qty_field="n_colli"  -> colli
    - qty_field="pezzo"/"pezzi" -> pezzi

    Presente in un giorno = data_ingresso <= giorno AND (data_uscita è vuota oppure data_uscita > giorno)
    """
    from collections import defaultdict
    from datetime import timedelta, date, datetime

    qty_per_giorno = defaultdict(float)
    ...
    def get_qty(art):
        # prova più nomi: pezzi/pezzo oppure n_colli
        if qty_field.lower() in ("pezzo", "pezzi"):
            v = getattr(art, "pezzi", None)
            if v is None:
                v = getattr(art, "pezzo", None)
        else:
            v = getattr(art, "n_colli", None)
    ...
    for art in articoli:
        ...
        while curr <= end:
            ...
            if presente:
                qty_per_giorno[(cliente_key, curr)] += qty
            curr += timedelta(days=1)

    risultati = []

    if raggruppamento == "giorno":
        keys = sorted(qty_per_giorno.keys(), key=lambda k: (k[0], k[1]))
        for cli, day in keys:
            v = qty_per_giorno[(cli, day)]
            risultati.append({
        ...
        agg = defaultdict(lambda: {"sum": 0.0, "days": set()})
        for (cli, day), v in qty_per_giorno.items():
            k = (cli, day.year, day.month)
            agg[k]["sum"] += v
            agg[k]["days"].add(day)
