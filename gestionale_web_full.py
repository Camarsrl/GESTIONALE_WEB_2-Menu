...
# ----------------- UTILS -----------------
def to_float_eu(v):
    """'1,23' → 1.23 ; ''/None → None; gestisce anche '1.234,56'."""
    if v is None:
        return None
    ...
    if not s:
        return None
    # ultima virgola come separatore decimale
    if "," in s and "." in s:
        if s.rfind(",") > s.rfind("."):
            s = s.replace(".", "").replace(",", ".")
        else:
...
<div class='row g-3'>
  ...
  <div class='col-md-9'>
    <div class='card p-4'>
      <h4>Benvenuto</h4>
      <p class='text-muted'>Stampe HTML e PDF, profili import, progressivo DDT.</p>
    </div>
  </div>
</div>
{% endblock %}"""
...
def scarico_ddt():
    ...
    # PDF DDT
    doc, story, bio = _pdf_doc(f"Documento di Trasporto (DDT) n. {n_ddt} del {fmt_date(today)}")
    data = [['ID', 'Cod.Art.', 'Descrizione', 'Colli', 'Peso', 'Commessa', 'Ordine']]
    tot_colli = 0
    tot_peso = 0.0
    for r in rows:
        tot_colli += (r.n_colli or 0)
        tot_peso += (r.peso or 0.0)
        data.append([r.id_articolo, r.codice_articolo or '', r.descrizione or '',
                     r.n_colli or 1, r.peso or '', r.commessa or '', r.ordine or ''])
    data.append(['', '', 'TOTALE', tot_colli, round(tot_peso,2), '', ''])
    story.append(_pdf_table(data, col_widths=[15*mm, 25*mm, 80*mm, 15*mm, 20*mm, 30*mm, 25*mm]))
    doc.build(story)
    bio.seek(0)
    return send_file(bio, as_attachment=True, download_name=f'ddt_{n_ddt.replace("/","-")}.pdf')
